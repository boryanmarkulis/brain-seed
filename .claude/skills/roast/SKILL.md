---
name: roast
description: Use when someone floats or asks to validate a NEW idea (business, product, offer), asks to roast or pressure-test an idea, "convene the council", wants a brutal second opinion before building something, or says "/roast". Entry point for idea validation - if the idea's context is incomplete it runs a grill-me discovery session first, then spins up a 6-persona council that attacks the idea from every angle, and a Judge returns one GO / RESHAPE / KILL verdict with the cheapest test to de-risk it.
...
## Step 1: Get the brief (grill only if needed)

If `$ARGUMENTS` contains the idea, start there. Then judge how complete the picture already is, from the conversation and from the Brain, and route:

- **Context is all there** (idea, buyer + money model, edge, constraints are clear or findable in the Brain): skip all questions and go straight to the council.
- **A few small gaps**: ask a tight batch of clarifying questions, 3-4 max, one round only:
...
- **The idea is genuinely fuzzy** (lots to figure out, dependent decisions, more in the user's head than on the page): invoke the `grill-me` skill first and run a full discovery session on the idea. When the grill is done, distill its capture file into the brief and continue here with the council.

If the user says "just run it", skip everything and proceed with what you have. Don't over-interrogate.
